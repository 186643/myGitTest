...
$list_num = 15;
$qsql="and wap_ok='1'";//mobile
$where ="where lang='$lang' {$qsql} order by no_order";
if($search=='detail_search' && $ftype!='all'){
	$tp = $ftype==1?"and img_path!=''":"and flash_path!=''";
	$where ="where lang='$lang' {$qsql} {$tp} order by no_order";
}
$ftype1[$ftype]='selected';
if($module<>""){
	$dule = $met_flasharray[$module]['type']==2?"(flash_path !='' and module = 'metinfo')":"(img_path !='' and module = 'metinfo')";
	$where ="where lang='$lang' {$qsql} and (module like '%,{$module},%' or {$dule}) order by no_order";
...
include template('app/wap/flash');
footer();
# Copyright (C) Lystk1958 Co., Ltd. (http://www.lystk1958.com.cn). All rights reserved.
?>
